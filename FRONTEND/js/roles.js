...
async function cargarUsuarios() {
  ...
  try {
    ...
    if (!usuarios.length) {
      tbody.innerHTML = '<tr><td colspan="4" class="table-loading">No hay usuarios.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = usuarios.map(u => {
      ...
      return `
        <tr data-usuario-id="${u.id}">
          ...
          <td>
            <button class="btn-table btn-disable btn-eliminar-usuario" data-usuario-id="${u.id}" data-nombre="${u.nombre} ${u.apellido}">
              Eliminar
            </button>
          </td>
        </tr>`;
